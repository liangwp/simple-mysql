...
function jsonifySQLResult(SQLResult) {
    ...
    // console.log(SQLResult); // contains a bunch of functions to get metadata.

    // current routine works for mysqlx.getVersion() == 8.0.22
    // (note, current version is pinned in package.json)

    // console.log('======== getColumns');
    var column_names = SQLResult.getColumns().map(colmeta => {
        return colmeta.getColumnLabel()
    });
    // console.log('======== hasData');
    // var hasdata = 
    // console.log(hasdata);
    ...
    if (SQLResult.hasData()) {
        ...
        returnResult = SQLResult.fetchAll().map(row => {
            var row_obj = {};
            for (let i = 0; i<row.length; i++) {
                row_obj[column_names[i]] = row[i];
            }
            return row_obj;
        });
    } else {
    ...
    returnResult.rows = returnResult.length;
    returnResult.column_names = column_names;
    return returnResult;
}
...
TransactionWrapper.prototype.commit = function () {
    // probably should auto-rollback if no commit in some amount of time
